...
void SDMATest::SetUp() {
    Platform::ChooseVariant(Platform::kPlatformEnclaveGuest);
    auto &plat =
        static_cast<enclave::EnclaveGuestPlatform &>(Platform::Instance());
    enclave::EnclaveGuestPlatform::Options options;
    options.shm_fn = getenv("G6_ENCLAVE_SHM");
    plat.SetOptions(options);
