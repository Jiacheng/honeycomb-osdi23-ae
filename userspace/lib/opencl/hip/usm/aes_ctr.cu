...
#define DECLSPEC __device__ inline
...
DECLSPEC void aes256_set_encrypt_key (PRIVATE_AS u32 *ks, PRIVATE_AS const u32 *ukey, SHM_TYPE u32 *s_te0, SHM_TYPE u32 *s_te1, SHM_TYPE u32 *s_te2, SHM_TYPE u32 *s_te3)
{
  ...
  ukey_s[0] = __builtin_bswap32 (ukey[0]);
  ukey_s[1] = __builtin_bswap32 (ukey[1]);
  ukey_s[2] = __builtin_bswap32 (ukey[2]);
  ukey_s[3] = __builtin_bswap32 (ukey[3]);
  ukey_s[4] = __builtin_bswap32 (ukey[4]);
  ukey_s[5] = __builtin_bswap32 (ukey[5]);
  ukey_s[6] = __builtin_bswap32 (ukey[6]);
  ukey_s[7] = __builtin_bswap32 (ukey[7]);
...
DECLSPEC void aes256_encrypt (PRIVATE_AS const u32 *ks, PRIVATE_AS const u32 *in, PRIVATE_AS u32 *out, SHM_TYPE u32 *s_te0, SHM_TYPE u32 *s_te1, SHM_TYPE u32 *s_te2, SHM_TYPE u32 *s_te3, SHM_TYPE u32 *s_te4)
{
  ...
  in_s[0] = __builtin_bswap32 (in[0]);
  in_s[1] = __builtin_bswap32 (in[1]);
  in_s[2] = __builtin_bswap32 (in[2]);
  in_s[3] = __builtin_bswap32 (in[3]);
  ...
  out[0] = __builtin_bswap32 (out[0]);
  out[1] = __builtin_bswap32 (out[1]);
  out[2] = __builtin_bswap32 (out[2]);
  out[3] = __builtin_bswap32 (out[3]);
}

KERNEL_FQ void AES256_CTR_init(u32 *ks, const u32 *ukey) {
    aes256_set_encrypt_key (ks, ukey, te0, te1, te2, te3);
}

KERNEL_FQ void AES256_CTR_compute_unaligned(u8 *out, const u8 *in, const u32 *iv, const u32 *ks, const u64 size_bytes, const u64 iv_start) {
    enum {
        kBlockSizeX = 256,
    };
    u64 tid = threadIdx.x;
    u64 gid = threadIdx.x + blockIdx.x * kBlockSizeX;
    if (gid != 0) {
        return;
    }
    u64 iv_counter = iv_start + gid;

    u32 counter[4];
    // do not change endianness for nonce (to match openssl)
    counter[0] = iv[0];
    counter[1] = iv[1];
    // make counter big endian (match openssl...)
    counter[2] = iv[2] + __builtin_bswap32(iv_counter >> 32);
    counter[3] = iv[3] + __builtin_bswap32(iv_counter & 0xFFFFFFFF);

    u32 encrypted[4];
    // this is exactly openssl AES_encrypt
    aes256_encrypt(ks, counter, encrypted, te0, te1, te2, te3, te4);

    // only process up to 15 bytes
    for (int i = 0; i != (size_bytes & 0xF); ++i) {
        // little endian
        u32 word_idx = i >> 2;
        u32 byte_idx = i & 0x3;
        u32 byte = ((encrypted[word_idx] >> (byte_idx << 3)) & 0xFF);
        out[i] = in[i] ^ byte;
    }
}

KERNEL_FQ void AES256_CTR_compute(u32 *out, const u32 *in, const u32 *iv, const u32 *ks, const u64 size_bytes, const u64 iv_start) {
    enum {
        kBlockSizeX = 256,
        kAESBlockSizeLog2 = 4, // log4(16) = 4
    };
    u64 tid = threadIdx.x;
    u64 gid = threadIdx.x + blockIdx.x * kBlockSizeX;
    u64 iv_counter = iv_start + gid;
    ...
    int blocks = size_bytes >> kAESBlockSizeLog2;
    if (gid < blocks) {
        out[gid * 4 + 0] = in[gid * 4 + 0] ^ encrypted[0];
        out[gid * 4 + 1] = in[gid * 4 + 1] ^ encrypted[1];
        out[gid * 4 + 2] = in[gid * 4 + 2] ^ encrypted[2];
        out[gid * 4 + 3] = in[gid * 4 + 3] ^ encrypted[3];
    }
}
