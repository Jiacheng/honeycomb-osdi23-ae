...
struct ConfigurationSpaceLayout {
    ...
    enum {
        ...
        kTXBufferWatermarkOffset = 4096 + sizeof(struct Watermark),
...
static int ivpci_open(struct inode *inode, struct file *filp)
{
    ...
    BUG_ON(ivpci_dev->bar2_len < SHM_SIZE);
    memcpy(sh_mem, ivpci_dev->base_addr, 4096); // should be sizeof(ConfigurationSpace)
...
static int ivpci_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    if (off < SHM_SIZE) {
        ...
        page = virt_to_page((unsigned long)sh_mem + off); 
...
{
    struct ivpci_private *ivpci_dev;
    // kernel to userspace, tx/rx against kernel
    unsigned long *kernel_rx_rptr, *kernel_rx_wptr,
    	*kernel_tx_rptr, *kernel_tx_wptr;
    // pcie to kernel, tx/rx against pcie
    unsigned long *pcie_rx_rptr, *pcie_rx_wptr,
    	*pcie_tx_rptr, *pcie_tx_wptr;
    unsigned char *kernel_rx_buffer, *kernel_tx_buffer;
    unsigned char *pcie_rx_buffer, *pcie_tx_buffer;

    unsigned long kernel_rx_rptr_value, kernel_rx_wptr_value;
    unsigned long kernel_tx_rptr_value, kernel_tx_wptr_value;
    unsigned long pcie_rx_rptr_value, pcie_rx_wptr_value;
    unsigned long pcie_tx_rptr_value, pcie_tx_wptr_value;

#define RPC_MAX_SIZE 256
    char rpc[RPC_MAX_SIZE];
    unsigned long rpc_size;
    ...
    kernel_rx_rptr = (unsigned long *)(sh_mem + kRXBufferWatermarkOffset);
    kernel_rx_wptr = (unsigned long *)(sh_mem + kRXBufferWatermarkOffset + sizeof(unsigned long));
    kernel_tx_rptr = (unsigned long *)(sh_mem + kTXBufferWatermarkOffset);
    kernel_tx_wptr = (unsigned long *)(sh_mem + kTXBufferWatermarkOffset + sizeof(unsigned long));
    kernel_rx_buffer = sh_mem + kRXBufferOffset;
    kernel_tx_buffer = sh_mem + kTXBufferOffset;

    pcie_rx_rptr = (unsigned long *)(ivpci_dev->base_addr + kRXBufferWatermarkOffset);
    pcie_rx_wptr = (unsigned long *)(ivpci_dev->base_addr + kRXBufferWatermarkOffset + sizeof(unsigned long));
    pcie_tx_rptr = (unsigned long *)(ivpci_dev->base_addr + kTXBufferWatermarkOffset);
    pcie_tx_wptr = (unsigned long *)(ivpci_dev->base_addr + kTXBufferWatermarkOffset + sizeof(unsigned long));
    pcie_rx_buffer = ivpci_dev->base_addr + kRXBufferOffset;
    pcie_tx_buffer = ivpci_dev->base_addr + kTXBufferOffset;

    // get from userspace
    // FIXME: atomic
    kernel_rx_rptr_value = *kernel_rx_rptr;
    kernel_rx_wptr_value = *kernel_rx_wptr;
    if (kernel_rx_rptr_value != kernel_rx_wptr_value) {
        if (kernel_rx_rptr_value < kernel_rx_wptr_value) {
            rpc_size = kernel_rx_wptr_value - kernel_rx_rptr_value;
            BUG_ON(rpc_size > RPC_MAX_SIZE);
            memcpy(rpc, kernel_rx_buffer + kernel_rx_rptr_value, rpc_size);
        } else {
            rpc_size = RX_BUFFER_SIZE - kernel_rx_rptr_value + kernel_rx_wptr_value;
            BUG_ON(rpc_size > RPC_MAX_SIZE);
            memcpy(rpc, kernel_rx_buffer + kernel_rx_rptr_value, RX_BUFFER_SIZE - kernel_rx_rptr_value);
            memcpy(rpc + RX_BUFFER_SIZE - kernel_rx_rptr_value, kernel_rx_buffer, kernel_rx_wptr_value);
        }
        *kernel_rx_rptr = kernel_rx_wptr_value;
        //pr_info(PFX "read type %d, size %ld\n", rpc[0], rpc_size);
    }

    // push to pcie
    pcie_rx_wptr_value = *pcie_rx_wptr;
    if (pcie_rx_wptr_value + rpc_size < RX_BUFFER_SIZE) {
        memcpy(pcie_rx_buffer + pcie_rx_wptr_value, rpc, rpc_size);
        *pcie_rx_wptr = pcie_rx_wptr_value + rpc_size;
    } else {
        memcpy(pcie_rx_buffer + pcie_rx_wptr_value, rpc, RX_BUFFER_SIZE - pcie_rx_wptr_value);
        memcpy(pcie_rx_buffer, rpc + RX_BUFFER_SIZE - pcie_rx_wptr_value, rpc_size + pcie_rx_wptr_value - RX_BUFFER_SIZE);
        *pcie_rx_wptr = rpc_size + pcie_rx_wptr_value - RX_BUFFER_SIZE;
    }
    //pr_info(PFX "push to pcie, rx wptr %lX\n", *pcie_rx_wptr);

    pcie_tx_rptr_value = *pcie_tx_rptr;
    pcie_tx_wptr_value = *pcie_tx_wptr;
    //pr_info(PFX "pcie tx rptr %lX wptr %lX\n", pcie_tx_rptr_value, pcie_tx_wptr_value);
    while (pcie_tx_rptr_value == pcie_tx_wptr_value) {
        yield();
        pcie_tx_wptr_value = *pcie_tx_wptr;
    }

    // get from pcie
    // use RPC for response
    if (pcie_tx_rptr_value < pcie_tx_wptr_value) {
        rpc_size = pcie_tx_wptr_value - pcie_tx_rptr_value;
        BUG_ON(rpc_size > RPC_MAX_SIZE);
        memcpy(rpc, pcie_tx_buffer + pcie_tx_rptr_value, rpc_size);
    } else {
        rpc_size = RX_BUFFER_SIZE - pcie_tx_rptr_value + pcie_tx_wptr_value;
        BUG_ON(rpc_size > RPC_MAX_SIZE);
        memcpy(rpc, pcie_tx_buffer + pcie_tx_rptr_value, RX_BUFFER_SIZE - pcie_tx_rptr_value);
        memcpy(rpc + RX_BUFFER_SIZE - pcie_tx_rptr_value, pcie_tx_buffer, pcie_tx_wptr_value);
    }
    *pcie_tx_rptr = pcie_tx_wptr_value;
    //pr_info(PFX "response type %d, size %ld\n", rpc[0], rpc_size);

    // push to user
    kernel_tx_wptr_value = *kernel_tx_wptr;
    if (kernel_tx_wptr_value + rpc_size < RX_BUFFER_SIZE) {
        memcpy(kernel_tx_buffer + kernel_tx_wptr_value, rpc, rpc_size);
        *kernel_tx_wptr = kernel_tx_wptr_value + rpc_size;
    } else {
        memcpy(kernel_tx_buffer + kernel_tx_wptr_value, rpc, RX_BUFFER_SIZE - kernel_tx_wptr_value);
        memcpy(kernel_tx_buffer, rpc + RX_BUFFER_SIZE - kernel_tx_wptr_value, rpc_size + kernel_tx_wptr_value - RX_BUFFER_SIZE);
        *kernel_tx_wptr = rpc_size + kernel_tx_wptr_value - RX_BUFFER_SIZE;
    }
...
static int ivpci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    dev_info(&pdev->dev, PFX "BAR0: 0x%0lx, %ld\n", ivpci_dev->bar0_addr,
            ivpci_dev->bar0_len);
    dev_info(&pdev->dev, PFX "BAR1: 0x%0lx, %ld\n", ivpci_dev->bar1_addr,
            ivpci_dev->bar1_len);
    dev_info(&pdev->dev, PFX "BAR2: 0x%0lx, %ld\n", ivpci_dev->bar2_addr,
            ivpci_dev->bar2_len);

    ivpci_dev->regs_addr = ioremap(ivpci_dev->bar0_addr, ivpci_dev->bar0_len);
    if (!ivpci_dev->regs_addr) {
        dev_err(&pdev->dev, PFX "unable to ioremap bar0, size: %ld\n",
                ivpci_dev->bar0_len);
    ...
    if (!ivpci_dev->base_addr) {
        dev_err(&pdev->dev, PFX "unable to ioremap bar2, size: %ld\n",
                ivpci_dev->bar2_len);
        goto iounmap_bar0;
    }
...
static int __init ivpci_init(void)
{
    ...
    sh_mem = kmalloc(SHM_SIZE, GFP_KERNEL);
    if (sh_mem == NULL) {
        ret = -ENOMEM; 
        goto out;
    }
