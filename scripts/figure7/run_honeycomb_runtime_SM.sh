...
> "${RESULT_DIR}/ACCEL_SM_output.csv"
...
while IFS=':' read -r filepath content; do
  ...
  echo "${program},${content}," >> "${RESULT_DIR}/ACCEL_SM_output.csv"
done < "${RESULT_DIR}/time.txt"

> "${RESULT_DIR}/Resnet18_SM_output.csv"
...
while IFS=':' read -r filepath content; do
  program=$(basename "${filepath}")
  echo "resnet,${content}," >> "${RESULT_DIR}/Resnet18_SM_output.csv"
done < "${RESULT_DIR}/time.txt"